...
function Server({ origin, hostname, host, port, file }) {
  ...
  server.listen = (...args) => {
    closed = false;
    streamDebug(`Listening stream server`);
  ...
  server.close = (cb) => {
    ...
    streamDebug(`Closing stream server`);
    _close.call(server, cb);
  };

  server.destroy = (cb) => {
    sockets.forEach((socket) => {
      streamDebug(`Destroy socket`);
      socket.destroy();
    });
  ...
  function onRequest(req, res) {
    ...
    if (
      opts.hostname &&
      req.headers.host !== `${opts.hostname}:${server.address().port}`
    ) {
      return req.destroy();
    }
    ...
    res.setHeader(
      ...
      "base-uri 'none'; default-src 'none'; frame-ancestors 'none'; form-action 'none';"
    );
    ...
    function serveFile(file) {
      ...
      pump(createReadStream(file.path), res);
    }
...
module.exports = function ({ origin, hostname, host, port }) {
  let server;

  return {
    async start(mediaFile) {
      streamDebug(`Create stream server`);
      server = new Server({ origin, hostname, host, port, file: mediaFile });
      const media = await promisify(server.listen.bind(server))(
        port,
        host
      ).then(() => ({
        host,
    ...
    async streamerShutdown() {
      streamDebug(`Shutdown stream server: ${!!server}`);
      try {
        if (!server) return true;
        streamDebug(`Stream server have found`);
        server.destroy();
        streamDebug(`Stream server has destroyed`);
      } catch (error) {
        streamDebug(`Error: ${error.message}`);
      } finally {
        server = null;
