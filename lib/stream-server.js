...
function Server({ origin, hostname, host, port }) {
  let file;
  ...
  server.start = (mediaFile) => {
    file = mediaFile;
    return promisify(server.listen.bind(server))(opts.port, opts.host).then(
      () => ({
        host,
        port,
        file,
  ...
  server.stop = () => promisify(server.close.bind(server))();
  server.streamerShutdown = () => promisify(server.destroy.bind(server))();
