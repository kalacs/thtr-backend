...
async function initClient({ downloadPath, filePath, client }) {
  ...
  /*  addOrSeed({
    downloadPath,
    client,
    file: "torrentFiles/[nCore][hd]Talpig_fegyverben_1080p.torrent"
  });
*/
  const torrent = await addTorrent(
    ...
    "torrentFiles/[nCore][hd]Talpig_fegyverben_1080p.torrent"
  );
  console.log(torrent.announce);
  // torrents.map(file => addOrSeed({ downloadPath, client, file }));
  // 2. seed if downloaded
  // 3. watch for new torrents
}
...
function addToSeed({ client, name, downloadPath }) {
  return new Promise(resolve => {
    client.seed(
      `${downloadPath}/${name}`,
      {
...
async function addOrSeed({ downloadPath, client, file }) {
  const torrent = await addTorrent({ downloadPath, client }, file);
  const { done, name } = torrent;

  if (done) return addToSeed({ downloadPath, client, name });
  return torrent;
}
